...
[modelR22_flavo_MADD,~,~,~] = deleteModelGenes(modelR3D_flavo, gene);
%save temp_workspace
%reduce models
...
rModel_flavo = reduceModel(modelR3D_flavo);
...
% Recon22, Recon22_MADD, Recon22_FAD, Recon22_FAD_MADD, Recon22_flavo, Recon22_flaco_MADD 

rxnList = {'biomass_reaction', 'ETF'};
sampleMADD_R22 =[];
for i = 1:length(rxnList)
    id = findRxnIDs(sModel_R, rxnList(i));
    sampleMADD_R22 = [sampleMADD_R22 sModel_R.points(id, :)'];
    id = findRxnIDs(sModel_R_MADD, rxnList(i));
    if id == 0
        sampleMADD_R22 = [sampleMADD_R22 zeros(10000,1)];
    else
        sampleMADD_R22 = [sampleMADD_R22 sModel_R_MADD.points(id, :)'];
    end
    id = findRxnIDs(sModel_FAD, rxnList(i));
    sampleMADD_R22 = [sampleMADD_R22 sModel_FAD.points(id, :)'];
    id = findRxnIDs(sModel_FAD_MADD, rxnList(i));
    if id == 0
        sampleMADD_R22 = [sampleMADD_R22 zeros(10000,1)];
    else
        sampleMADD_R22 = [sampleMADD_R22 sModel_FAD_MADD.points(id, :)'];
    end
    id = findRxnIDs(sModel_flavo, rxnList(i));
    sampleMADD_R22 = [sampleMADD_R22 sModel_flavo.points(id, :)'];
    id = findRxnIDs(sModel_flavo_MADD, rxnList(i));
    if id == 0
        sampleMADD_R22 = [sampleMADD_R22 zeros(10000,1)];
    else
        sampleMADD_R22 = [sampleMADD_R22 sModel_flavo_MADD.points(id, :)'];
    end
end
...
sampleMADD_R22 = [sampleMADD_R22 sum(Ctrl,1)'];
sampleMADD_R22 = [sampleMADD_R22 sum(Ctrl_MADD,1)'];
sampleMADD_R22 = [sampleMADD_R22 sum(FAD,1)'];
sampleMADD_R22 = [sampleMADD_R22 sum(FAD_MADD,1)'];
sampleMADD_R22 = [sampleMADD_R22 sum(FAD_flavo,1)'];
sampleMADD_R22 = [sampleMADD_R22 sum(FAD_flavo_MADD,1)'];
